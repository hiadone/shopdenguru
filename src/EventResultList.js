...
function EventResultList(props) {
  let [eventResultListData, eventResultListDataFunc] = useState(false);
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: "https://api.denguru.kr/mypage/resultevent",
      headers: {
        Authorization: cookie.accessToken ? cookie.accessToken : ""
      }
    })
      .then(res => {
        eventResultListDataFunc(res.data);
      })
      .catch(() => {
        console.log("실패햇어요");
      });
      ...
      {eventResultListData === false ? (
        ...
        <>
          <header className="header03 header03_white">
            <h1 className="h_title">이벤트 당첨 내역</h1>
          ...
          <div className="main">
            ...
            <div className="tab_nav02">
              <Link to="/eventapplylist" className="tab_nav_link ">
                <span>신청한 이벤트</span>
              </Link>
              <Link to="/eventresultlist" className="tab_nav_link active">
                <span>당첨된 이벤트</span>
              </Link>
            </div>
            <div className="event_list_container">
              <ul className="event_list">
                {eventResultListData.data.list.map(val => (
